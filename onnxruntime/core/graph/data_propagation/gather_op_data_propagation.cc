      ...
      if (indices.size() == 1) {
        ...
        if (dim.has_dim_value()) {
          ...
          switch (ClassifySingleValueRank(effective_num_dims)) {
            case SingleValueRank::kScalar:
              SetSinglePropagatedShapeValue(output_def_, dim.dim_value(), /*is_rank1=*/false);
              break;
            case SingleValueRank::kRank1:
              SetSinglePropagatedShapeValue(output_def_, dim.dim_value(), /*is_rank1=*/true);
              break;
            case SingleValueRank::kDecline:
              // rank >= 2 or unknown index rank: leave the output unset and let ONNX data
              // propagation handle it.
              break;
          }
        }
      }
