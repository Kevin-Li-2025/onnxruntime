...
// the model at every graph-optimization level and asserts the rank-1 K shape is preserved so the
...
TEST(ShapeInferenceV2Test, GatherToTopKRankPreservationTest) {
  ...
  for (auto opt_level : {ORT_DISABLE_ALL, ORT_ENABLE_BASIC, ORT_ENABLE_EXTENDED, ORT_ENABLE_LAYOUT, ORT_ENABLE_ALL}) {
    Ort::SessionOptions session_options{};
    session_options.SetGraphOptimizationLevel(opt_level);

    // Loading must succeed at every optimization level; before the fix this threw a
...
// Unit test for ClassifySingleValueRank, the rank-routing used by Gather's custom data propagation
// to decide how a single extracted element is represented. A 0-D scalar index yields a scalar
// output value, a 1-D index yields a rank-1 [1] value, and a rank >= 2 index (or an unknown rank,
// sentinel < 0) DECLINES -- the single-value channel cannot represent a rank >= 2 Gather output, so
// fabricating a rank-1 value would misreport the rank to downstream consumers. Relaxing the
// rank >= 2 / unknown branch to scalarize or rank-1 fails the decline assertions below.
TEST(ShapeInferenceV2Test, ClassifySingleValueRankRoutingTest) {
  EXPECT_EQ(ClassifySingleValueRank(0), SingleValueRank::kScalar);
  EXPECT_EQ(ClassifySingleValueRank(1), SingleValueRank::kRank1);
  // rank-2 single-element index (e.g. shape [1,1]) -> decline, not a fabricated rank-1 value.
  EXPECT_EQ(ClassifySingleValueRank(2), SingleValueRank::kDecline);
  EXPECT_EQ(ClassifySingleValueRank(3), SingleValueRank::kDecline);
  // Unknown rank (sentinel < 0) -> decline rather than guess.
  EXPECT_EQ(ClassifySingleValueRank(-1), SingleValueRank::kDecline);
}

// Unit test for ShouldDeclineUnsqueezeSingleValue, the pure predicate that locks Unsqueeze's
// decline-on-unrepresentable decision. A single-element value (a rank-1 [1] scalar-like quantity)
// unsqueezes to rank >= 2, which the single-value channel cannot represent, so the predicate
// returns true (decline). Multi-element shape vectors and the empty case are legitimate and must
// NOT decline. This is the PRIMARY lock for the Unsqueeze decline: a graph-level test is
// structurally infeasible because the rank >= 2 result cannot flow to any legal ONNX shape-consumer
// (see the comment at the Unsqueeze decline). Relaxing the decline branch fails this test.
TEST(ShapeInferenceV2Test, ShouldDeclineUnsqueezeSingleValueTest) {
  // Single-element rank-1 value -> unsqueeze result is rank >= 2 -> decline.
  EXPECT_TRUE(ShouldDeclineUnsqueezeSingleValue(1));
  // Multi-element shape vectors are legitimate and must NOT decline.
  EXPECT_FALSE(ShouldDeclineUnsqueezeSingleValue(2));
  EXPECT_FALSE(ShouldDeclineUnsqueezeSingleValue(3));
  // An empty value vector is not the single-element scalar-like case.
  EXPECT_FALSE(ShouldDeclineUnsqueezeSingleValue(0));
}
